...
def plot_scatter(x_train, y_train, x_test, y_test):
    ...
    fig.update_layout(autosize=True, height=500, width = 500, margin=dict(l=5, r=10, b=30, t=10), legend=dict(font=dict(size=14, family="sans-serif", color="#FAFAFA"), bgcolor="rgba(0, 0, 0, 0)", orientation="h", yref="paper", yanchor="bottom", y=0, xref="paper", xanchor="right", x=1))
    fig.add_annotation(dict(font=dict(color="#FAFAFA", size=16, family="sans-serif"), yref="paper", yanchor="top", y=-0.02, xref="paper", xanchor="left", x=0.5, showarrow=False, text="X1"))
    return fig

def plot_scatter_decision_boundary(label, model, x_train, y_train, x_test, y_test):
    d = x_train.shape[1]
    x_min, x_max = x_train[:, 0].min() - 1, x_train[:, 0].max() + 1
    y_min, y_max = x_train[:, 1].min() - 1, x_train[:, 1].max() + 1
    ...
    fig.add_trace(heatmap).add_trace(train_data).add_trace(test_data).update_xaxes(range=[x_min, x_max]).update_yaxes(range=[y_min, y_max]).update_layout(xaxis=dict(tickfont=dict(size=14, family="sans-serif", color="#FAFAFA")), yaxis_title=dict(text="X2", font=dict(size=16, family="sans-serif", color="#FAFAFA")), yaxis=dict(tickfont=dict(size=14, family="sans-serif", color="#FAFAFA")))
 
    fig.update_xaxes(showticklabels=False, showline=True, showgrid=False, zeroline=False, linecolor = '#FAFAFA', linewidth = 2.5, mirror = True)
    fig.update_yaxes(showticklabels=False, showline=True, showgrid=False, zeroline=False, linecolor = '#FAFAFA', linewidth = 2.5, mirror = True)
    fig.update_layout(autosize=True, height=500, width = 500, margin=dict(l=5, r=10, b=4, t=10), legend=dict(font=dict(size=14, family="sans-serif", color="#FAFAFA"), bgcolor="rgba(0, 0, 0, 0)", orientation="h", yref="paper", yanchor="bottom", y=0, xref="paper", xanchor="right", x=1))
    fig.add_annotation(dict(font=dict(color="#FAFAFA", size=16, family="sans-serif"), yref="paper", yanchor="top", y=-0.04, xref="paper", xanchor="left", x=0.5, showarrow=False, text="X1"))
    fig.add_annotation(dict(font=dict(color="#FCBC24", size=25, family="sans-serif"), yref="paper", yanchor="top", y=0.98, xref="paper", xanchor="left", x=0.02, showarrow=False, text="{}".format(label)))
    return fig
